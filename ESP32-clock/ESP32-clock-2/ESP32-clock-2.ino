...
// Buzzer control (GPIO2 via LEDC PWM)
bool buzzerActive = false;
unsigned long buzzerStartMs = 0;
int lastBuzzMinute = -1;
...
void dht11read() {
  // Read DHT11 temperature and humidity sensor with error handling
  byte t = 0;
  byte h = 0;
...
void setup()
{
  ...
  // Initialize buzzer (tone on GPIO2)
  pinMode(LED_PIN, OUTPUT);
  noTone(LED_PIN);
  digitalWrite(LED_PIN, LOW); // ensure off

  // Ensure DHT11 input is pulled up (use external 4.7k~10k if available)
  pinMode(pinDHT11, INPUT_PULLUP);
  // Quick DHT11 probe so you can see readings early
  dht11read();
...
void loop()
{
  ...
  // Periodically read DHT11 (max ~1Hz for DHT11, we use 5s)
  ...
  // Buzzer: turn ON for 5 seconds at the top of every minute
  int currSec = second();
  int currMin = minute();
  if (currSec == 0 && lastBuzzMinute != currMin) {
    tone(LED_PIN, 2000); // start buzzer
    buzzerActive = true;
    buzzerStartMs = millis();
    lastBuzzMinute = currMin;
    Serial.println("BUZZER ON (start of minute)");
  }
  if (buzzerActive && millis() - buzzerStartMs >= 5000) {
    noTone(LED_PIN); // stop buzzer
