...
void initMotors() {
  // Motor LEDC channels setup (use same timer as servos to avoid conflicts)
  ledc_channel_config_t motor_channel_left_in1 = {};
  motor_channel_left_in1.gpio_num = LEFT_MOTOR_IN1;
  motor_channel_left_in1.speed_mode = servoSpeedMode;
  motor_channel_left_in1.channel = LEFT_MOTOR_IN1_CHANNEL;
  motor_channel_left_in1.timer_sel = servoTimer;
  motor_channel_left_in1.duty = 0;
  motor_channel_left_in1.hpoint = 0;
  motor_channel_left_in1.flags.output_invert = 0;
  ESP_ERROR_CHECK(ledc_channel_config(&motor_channel_left_in1));

  ledc_channel_config_t motor_channel_left_in2 = {};
  motor_channel_left_in2.gpio_num = LEFT_MOTOR_IN2;
  motor_channel_left_in2.speed_mode = servoSpeedMode;
  motor_channel_left_in2.channel = LEFT_MOTOR_IN2_CHANNEL;
  motor_channel_left_in2.timer_sel = servoTimer;
  motor_channel_left_in2.duty = 0;
  motor_channel_left_in2.hpoint = 0;
  motor_channel_left_in2.flags.output_invert = 0;
  ESP_ERROR_CHECK(ledc_channel_config(&motor_channel_left_in2));

  ledc_channel_config_t motor_channel_right_in1 = {};
  motor_channel_right_in1.gpio_num = RIGHT_MOTOR_IN1;
  motor_channel_right_in1.speed_mode = servoSpeedMode;
  motor_channel_right_in1.channel = RIGHT_MOTOR_IN1_CHANNEL;
  motor_channel_right_in1.timer_sel = servoTimer;
  motor_channel_right_in1.duty = 0;
  motor_channel_right_in1.hpoint = 0;
  motor_channel_right_in1.flags.output_invert = 0;
  ESP_ERROR_CHECK(ledc_channel_config(&motor_channel_right_in1));

  ledc_channel_config_t motor_channel_right_in2 = {};
  motor_channel_right_in2.gpio_num = RIGHT_MOTOR_IN2;
  motor_channel_right_in2.speed_mode = servoSpeedMode;
  motor_channel_right_in2.channel = RIGHT_MOTOR_IN2_CHANNEL;
  motor_channel_right_in2.timer_sel = servoTimer;
  motor_channel_right_in2.duty = 0;
  motor_channel_right_in2.hpoint = 0;
  motor_channel_right_in2.flags.output_invert = 0;
  ESP_ERROR_CHECK(ledc_channel_config(&motor_channel_right_in2));
...
void taskMotor(void* pv) {
  ...
  for (;;) {
    ControlData cmd;
    // Use xQueuePeek instead of xQueueReceive to allow both motor and servo tasks to read
    // ELRS task uses xQueueOverwrite to always keep latest data available
    if (xQueuePeek(motorCmdQueue, &cmd, pdMS_TO_TICKS(10)) == pdTRUE) {
      ...
      // Queue timeout - log warning periodically
      static uint32_t lastWarning = 0;
      if (millis() - lastWarning >= 5000) {
        LOG_WARN_MOTOR("No data in motorCmdQueue for 5+ seconds");
        lastWarning = millis();
      }
    }
...
void controlMotorsFromValues(uint16_t roll, uint16_t throttle) {
  ...
  if (roll == 0 && throttle == 0) {
    ...
    ledc_set_duty(servoSpeedMode, LEFT_MOTOR_IN1_CHANNEL, 0);
    ledc_update_duty(servoSpeedMode, LEFT_MOTOR_IN1_CHANNEL);
    ledc_set_duty(servoSpeedMode, LEFT_MOTOR_IN2_CHANNEL, 0);
    ledc_update_duty(servoSpeedMode, LEFT_MOTOR_IN2_CHANNEL);
    ledc_set_duty(servoSpeedMode, RIGHT_MOTOR_IN1_CHANNEL, 0);
    ledc_update_duty(servoSpeedMode, RIGHT_MOTOR_IN1_CHANNEL);
    ledc_set_duty(servoSpeedMode, RIGHT_MOTOR_IN2_CHANNEL, 0);
    ledc_update_duty(servoSpeedMode, RIGHT_MOTOR_IN2_CHANNEL);
  ...
  // Apply PWM
  uint32_t dutyLeftIn1 = (pwmLeftIn1 * 16383UL) / 255;
  uint32_t dutyLeftIn2 = (pwmLeftIn2 * 16383UL) / 255;
  uint32_t dutyRightIn1 = (pwmRightIn1 * 16383UL) / 255;
  uint32_t dutyRightIn2 = (pwmRightIn2 * 16383UL) / 255;

  ledc_set_duty(servoSpeedMode, LEFT_MOTOR_IN1_CHANNEL, dutyLeftIn1);
  ledc_update_duty(servoSpeedMode, LEFT_MOTOR_IN1_CHANNEL);
  ledc_set_duty(servoSpeedMode, LEFT_MOTOR_IN2_CHANNEL, dutyLeftIn2);
  ledc_update_duty(servoSpeedMode, LEFT_MOTOR_IN2_CHANNEL);
  ledc_set_duty(servoSpeedMode, RIGHT_MOTOR_IN1_CHANNEL, dutyRightIn1);
  ledc_update_duty(servoSpeedMode, RIGHT_MOTOR_IN1_CHANNEL);
  ledc_set_duty(servoSpeedMode, RIGHT_MOTOR_IN2_CHANNEL, dutyRightIn2);
  ledc_update_duty(servoSpeedMode, RIGHT_MOTOR_IN2_CHANNEL);
...
void controlRelays(const ControlData& cmd) {
  // Failsafe: If no RC signal at all (all channels including relay channels are zero), turn off all relays
  // This checks if we've lost connection completely
  if (cmd.steering == 0 && cmd.servoCh1 == 0 && cmd.throttle == 0 && 
      cmd.servoCh2 == 0 && cmd.relayCh6 == 0 && cmd.relayCh7 == 0) {
    digitalWrite(RELAY1_PIN, LOW);
    digitalWrite(RELAY2_PIN, LOW);
    digitalWrite(RELAY3_PIN, LOW);
    digitalWrite(RELAY4_PIN, LOW);
  ...
  // Control relays by channels from ControlData
  // Relays can be activated even if motors/servos are not moving
