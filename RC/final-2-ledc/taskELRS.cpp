...
void taskELRS(void* pv) {
  ...
  for (;;) {
    ...
    if (millis() - lastDebugPrint >= 2000) {
      LOG_DEBUG_ELRS("Updates/sec: %d | Packets/sec: %d | CH1(steering): %d | CH3(throttle): %d | CH5: %d | CH6(servo1): %d | CH7(servo2): %d | CH8: %d", 
                    updateCount / 2, (packetCount - lastPacketCount) / 2, ch1, ch3, ch5, ch6, ch7, ch8);
      
      // Additional debug: Check queue status
      UBaseType_t queueSpaces = uxQueueSpacesAvailable(motorCmdQueue);
      LOG_DEBUG_ELRS("Queue spaces available: %d (should be 0 for full queue)", queueSpaces);
