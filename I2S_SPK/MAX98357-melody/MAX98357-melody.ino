...
#define AMPLITUDE 9000  // Max 32767
...
void setup() {
  ...
  Serial.println();
  Serial.println("MAX98357 ESP32-S3 melody test start");
  Serial.printf("Pins: DIN=%d, LRC=%d, BCLK=%d\n", MAX_DIN, MAX_LRC, MAX_BCLK);
  Serial.printf("Audio: sample_rate=%d, amplitude=%d\n", SAMPLE_RATE, AMPLITUDE);
  ...
  Serial.println("I2S init done. Starting melody output.");
}
...
void loop() {
  ...
  static uint32_t noteStartedAt = millis();

  MelodyNote currentNote = melody[noteIndex];
  const float phaseIncrement = 2.0f * PI * currentNote.frequency / SAMPLE_RATE;

  if (millis() - noteStartedAt >= currentNote.durationMs) {
    noteIndex = (noteIndex + 1) % melodyLength;
    currentNote = melody[noteIndex];
    noteStartedAt = millis();
    phase = 0.0f;
    Serial.printf("Now playing note %u: %.2f Hz for %u ms\n", (unsigned int)noteIndex, currentNote.frequency, currentNote.durationMs);
  }

  for (int i = 0; i < BUFFER_LEN; i++) {
    int16_t sound = currentNote.frequency > 0.0f ? (int16_t)(AMPLITUDE * sin(phase)) : 0;
    int16_t silence = 0;
  ...
  if (now - last_debug_ms >= LOOP_DEBUG_INTERVAL_MS) {
    Serial.printf(
      "loop=%lu, write_result=%d, bytes_written=%u, note=%u, freq=%.2f, firstL=%d, firstR=%d, phase=%.4f\n",
