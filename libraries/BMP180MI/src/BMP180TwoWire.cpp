...
BMP180TwoWire::BMP180TwoWire(TwoWire *wire, uint8_t i2c_address) :
	wire_(wire),
...
BMP180TwoWire::~BMP180TwoWire()
{
	wire_ = nullptr;
}

bool BMP180TwoWire::beginInterface()
{
	return true;
}

uint8_t BMP180TwoWire::readRegister(uint8_t reg)
{
	if (!wire_)
		return 0;
...
uint32_t BMP180TwoWire::readRegisterBurst(uint8_t reg, uint8_t length)
{
	if (!wire_)
		return 0;
...
void BMP180TwoWire::writeRegister(uint8_t reg, uint8_t value)
{
	if (!wire_)
		return;
