...
void setup() {
	...
	{	//ESP32 does support high speed timers
		...
		Serial.println("  Esp32LedcRegistry contains the high speed timer. Tested OK");
		delay(1000);
	}
	...
	{	//timer clock frequency check
		...
		Serial.println("  ESP32 low speed default timer should be 80MHz. Tested OK");
		delay(1000);
	...
	
	{
		...
		ServoFactoryDecorator oFactoryDecorator(oTimerChannelFactory);	//let this ServoControllerPWMFactory define the frequency to use and such
	...
	
	{
		...
		PWMController oPwms[8];											//so 8 is the EPS32 value, this might not be a good plan on other controllers..
		const uint8_t uiPins[8] = {2,21,22,15,13,32,33,12};				//34, 35, 36, 39 are all input only
	...
	{
		Serial.println("Checking HighSpeedFactory functionality: ");
		const uint8_t 	uiPinNr1 = 2;
		const uint8_t	uiPinNr2 = 12;
		const uint32_t	uiFreqHz1 = 11000;
		const uint32_t	uiFreqHz2 = 22000;
		const double	dInitialDuty1 = 0.1;
		const double	dInitialDuty2 = 0.9;
		
		
		HighSpeedFactory oFactory;	//since we have the ESP32 now, lets use the best
		PWMController oPwm1;
		PWMController oPwm2;
		if ((!oPwm1.begin(oFactory, uiPinNr1, uiFreqHz1, dInitialDuty1)) ||
			(!oPwm2.begin(oFactory, uiPinNr2, uiFreqHz2, dInitialDuty2))) {
			Serial.println("  Failed to create two PWM Controllers with own timer");
			return;
		}
		Serial.println("  two PWM Controllers with own timer created");
		delay(5000);

		Serial.println("  fading those");
		for (int i=0; i<1000; i++) {
			double dDuty = (i==0) ? 0:(i/1000.0);
			oPwm1.fade(dDuty);				//non-blocking call
			oPwm2.fade(1.0-dDuty);			//non-blocking call
			delay(10);						//so we do need a delay
		}
		delay(20000);
